function spike_count_frequency = trains_to_spike_frequency( trains , trials)
...
% stores the probability that i-1 spikes occured for that stimulus. 
    ...
    spike_count_frequency = zeros(5, max(stimulus_max_spike_count) + 1); 
    % For each stimuli, 
    for i = 1:5
        ...
        for j = 1:stimulus_max_spike_count(i) + 1
            ...
            spike_count_frequency(i, j) = sum(spike_count(i, :) == j-1) / trials(i, 1);
        end
    end
end
